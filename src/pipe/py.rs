...
pub fn pipes(py: Python, m: &PyModule)  -> PyResult<()> {
    m.add_class::<Tcp>()?;
    Ok(())
}
...
impl Tcp {
    ...
    fn send(&mut self, py: Python, data: &[u8]) -> PyResult<usize> {
        Ok(self.tcp.send(data)?)
    }
    fn sendline(&mut self, py: Python, data: &[u8]) -> PyResult<usize> {
    ...
    fn debug(&mut self, py: Python) -> PyResult<()> {
        Ok(self.tcp.debug()?)
    }
    fn interactive(&mut self, py: Python) -> PyResult<()> {
